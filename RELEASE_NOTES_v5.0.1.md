...
**Root Cause**:
MatterSim calculator may perform internal structure relaxation when computing `get_potential_energy()`. This is a calculator-level behavior, not a bug in GOAD v5.0.
...
Potential enhancements in v5.1+:
...
### v5.0 (2025-11-28) - Initial Release
...
No special installation needed for v5.0.1. If updating from v5.0:
...
# Verify installation
python3 -m py_compile goad_v5/**/*.py
...
If using GOAD v5.0.1 in research, cite as:

```bibtex
@software{goad_v5_0_1,
  title={GOAD v5.0.1: Genetic Optimization for Adsorbed Structures},
  year={2025},
  version={5.0.1},
  url={...}
}
...
Thanks for reporting the single-point behavior issue. This feedback helps improve GOAD v5.0 for all users.
